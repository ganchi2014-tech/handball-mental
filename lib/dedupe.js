...
(function (root, factory) {
  ...
  function findRosterDuplicates(roster) {
    ...
    Object.entries(roster || {}).forEach(([id, r]) => {
      if (!r) return;
      if (!normalizeSurname(r.surname)) return;
      const key = dupKey(r);
  ...
  function planMerge(group, rosterToUid, chosenCanonicalId) {
    ...
    // 既定の統合先: 連携がちょうど1つならその行 → いなければ現役(active!==false)の先頭 → 先頭
    const firstActive = group.entries.find(e => e.active !== false);
    const canonicalId = (chosenCanonicalId && ids.includes(chosenCanonicalId))
      ? chosenCanonicalId
      : (linked.length === 1 ? linked[0].id : (firstActive ? firstActive.id : group.entries[0].id));
